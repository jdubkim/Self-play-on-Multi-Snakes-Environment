    ...
    def __init__(self, main_model, opponent_model):
        ...
        def multi_step(obs, opponent_obs, states, dones):
            ...
            if self.opponent_model == None:
                opponent_actions = [1 for i in range(len(actions))]
            else:
                opponent_actions, _, _, _ = self.opponent_model.step(opponent_obs, states, dones)

            self.full_actions = list(zip(actions, opponent_actions))
...
class Model(object):
    def __init__(self, *, policy, ob_shape, ac_space, nbatch_act,
                 nbatch_train, nsteps, ent_coef, vf_coef,
                 max_grad_norm, scope_name):

        sess = tf.get_default_session()
        act_model = policy(sess, ob_shape, ac_space, nbatch_act, 1,
                           scope_name, reuse=False)
        train_model = policy(sess, ob_shape, ac_space, nbatch_train,
                             nsteps, scope_name, reuse=True)
        ...
        vpredclipeed = OLDVPRED + tf.clip_by_value(train_model.vf - OLDVPRED, - CLIPRANGE, CLIPRANGE)
        vf_losses1 = tf.square(vpred - R)
        vf_losses2 = tf.square(vpredclipeed - R)
